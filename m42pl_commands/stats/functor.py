...
class StatsFunctor:
    ...
    def __init__(self, source_field: Field, aggr_fields: list, dest_field: Field, aggregates: dict):
        '''
        :param source_field:    Aggregation functor source field (must exist).
                                Ex: "| stats min(<source filed>) by ...
                                
        :param aggr_fields:     Aggregations fields.
                                Ex: "| stats min(...) by <aggr_fields>"
                                
        :param dest_field:      Aggregation functor result field (created or overwriten).
    ...
    def __call__(self, event, pipeline):
        ...
        for field in self.aggr_fields:
            ...
            field_value = str(field.read(event.data))
        ...
        aggregates[self.dest_field.name], value = self.target(event, aggregates[self.dest_field.name], pipeline)
        return value
